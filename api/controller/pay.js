const { order, user } = require('../base/baseModel');
module.exports = {
    /**
     * 获取支付的请求参数
    ...
    */
    ...
    'POST /pay/prepay': async (ctx, next) => {
        const orderId = ctx.get('orderId');
        const orderInfo = await order.where({ id: orderId }).find();
        if (orderInfo) {
        ...
        const openid = await user.where({ id: orderInfo.user_id }).getField('weixin_openid', true);
        if (openid) {
