const { product, goods_specification } = require('../base/baseModel');
module.exports = {
    ...
    async getProductList(goodsId) {
        const goods = await product.findAll({ where: { goods_id: goodsId } });
        return goods;
    },
    ...
    async getSpecificationList(goodsId) {
        // // 根据sku商品信息，查找规格值列表
        // const specificationRes = await goods_specification.alias('gs')
        //     .field(['gs.*', 's.name'])
        //     .join({
        //         table: 'specification',
        //         join: 'inner',
